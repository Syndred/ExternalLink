...
assert.match(source, /await\s+asyncio\.to_thread\(\s*deepseek_chat_json,\s*PLAN_PROMPT,\s*payload\s*\)/);
assert.match(source, /local_judge\(\s*payload\s*\)/);
assert.match(source, /await\s+asyncio\.to_thread\(\s*deepseek_chat_json,\s*JUDGE_PROMPT,\s*payload\s*\)/);

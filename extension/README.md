...
扩展已内置最近一次经核验的 Google Sheet 快照，安装或重载后即可离线查看、筛选和记录时间线，不需要先启动 Agent。需要拉取 Google Sheet 新改动时才启动本机 Agent，到 Settings → 外链库点击“检查 Google 更新”，确认后点“更新本地缓存”。更新缓存不会隐式回写 Google；成功账本回写使用独立的“回写待同步记录”按钮。断开 Google 也不会丢弃缓存。AI、媒体等本机能力同样只在使用时需要 Agent。扩展会保留 `Link Submit` 的全部原始列、每个 Profile 的全部 `Field / Content / Notes`。Google 暂时不可用时记录留在 outbox，可稍后重试。
...
- `Table.xlsx` / `table-library.json`：构建时生成的完整 Sheet 离线快照，不作为第二个人工维护入口。

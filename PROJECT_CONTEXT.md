...
- **2026-09-24 / 外链提交恢复与验收**：用户新建并充值 DeepSeek `outlink` Key；已写入 Cloudflare Worker Secret，部署 `4d293a19-d070-4f0a-abbd-0119736000a7`，直接模型请求 HTTP 200。扩展 3.7.6 已在 Ego 启用；10015 首步 URL 能填，但站点要求登录，已停放。Once Tools 的 JevPlay 实站提交收到明确回执，插件未自动记账，已按现场证据人工补记且云端回读；根因是 Profile `/games` 深链与表单官网根路径的精确匹配。3.7.8 已修复同站深链及纯文本 URL 表单监听，待重载实测。旧 `.env` 泄露 Key 已删除，新 Key 未入仓库。旧 `activeBatchRun` 冲突仍待处理，三产品各 30 站未完成。详见 `docs/外链提交验收与误填保护-2026-09-24.md`。
...
> 最后更新：2026-09-24｜扩展 3.7.8（源码；Ego 待重载）｜DeepSeek 新 Key 已接入 Worker｜路线：Neon 执行账本 + R2 证据 + 本地字段优先、AI 补全

...
- **2026-09-24 / 外链提交恢复与验收**：用户新建并充值 DeepSeek `outlink` Key；已写入 Cloudflare Worker Secret，部署 `4d293a19-d070-4f0a-abbd-0119736000a7`，直接模型请求 HTTP 200。Ego 已启用扩展 3.7.11。Insidr 与 Human or Not 已各完成三产品真实提交并云端回读；OldPhotoLive AI、Graffiti Name AI 的四次实站提交由插件自动登记，JevPlay 两次旧版提交人工补记。Once Tools 的 JevPlay 另有待审核回执并人工补记，但该站一次性付费软件定位不适合三产品覆盖。3.7.11 修复成功回执漏识别及跨 Profile 旧值回放。旧 `activeBatchRun` 冲突、重复动态与三产品各 30 站目标仍待处理。详见 `docs/外链提交验收与误填保护-2026-09-24.md`。
...
> 最后更新：2026-09-24｜扩展 3.7.11（Ego 已回读）｜DeepSeek 新 Key 已接入 Worker｜路线：Neon 执行账本 + R2 证据 + 本地字段优先、AI 补全

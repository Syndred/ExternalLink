...
receipt = hooks.classifyVisibleEvidence({ destinationUrl: "https://alieradox.com/submit" });
assert.equal(receipt.matched, false, "Alieradox review copy before submission is not a receipt");
document.body.innerText = "Submit an AI Tool Tool submitted successfully! Our team will review it shortly.";
receipt = hooks.classifyVisibleEvidence({ destinationUrl: "https://alieradox.com/submit" });
assert.equal(receipt.matched, true, "Alieradox's visible AJAX success notification proves submission");
assert.equal(receipt.evidence, "Tool submitted successfully! Our team will review it shortly.");
receipt = hooks.classifyVisibleEvidence({ destinationUrl: "https://another-directory.example/submit" });
assert.equal(receipt.matched, false, "Alieradox receipt must not be attributed to another source");
document.body.innerText = "Submit an AI Tool We review all submissions to ensure quality and relevance.";
receipt = hooks.classifyVisibleEvidence({ destinationUrl: "https://alieradox.com/submit" });
assert.equal(receipt.matched, false, "a dismissed Alieradox success notification must not count as a new receipt");

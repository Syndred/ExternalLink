...
> 最后更新：2026-09-08｜扩展 3.1.0｜路线：云端单一数据源
...
> **目录站无验证码可代点提交**（设置里默认开，批量页「仅填表」可关）。有验证码的页签留下等人，不占并发。登录 / OTP / 付费仍停。点完不算成功，要看到回执 / 待审核 / 已上线才写账本，然后开下一站。标准 WordPress 评论仍需另开开关才会代点。
...
- **使用前**：重载扩展 **3.1.0**；云端连接、状态首迁移和媒体首迁移已完成。检查队列质量闸门、「目录站无验证码可代点提交」（默认开）和标准评论代点开关（默认关）。

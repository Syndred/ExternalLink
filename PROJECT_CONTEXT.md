...
- 旧版扩展重启时可能自动重放本地待上传队列；Neon 修订冲突能保护已更新的 `siteProfiles`，但其他数据类若修订未变仍可能被写入。3.7.45 启动先回读云端，待上传/冲突期间暂停自动上传，待明确回读覆盖或手动成功上传后放行。台式机必须先完整退出旧浏览器，再拉代码、重载新版、回读云端；Ego 实机尚未验收。详见 `docs/整站自动提交与原创投稿交接-2026-09-25.md`。

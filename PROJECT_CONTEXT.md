...
- **2026-09-24 / 外链提交恢复与验收**：用户新建并充值 DeepSeek `outlink` Key；已写入 Cloudflare Worker Secret，当前部署 `4d293a19-d070-4f0a-abbd-0119736000a7`，新 Key 的 DeepSeek API 请求返回 HTTP 200。扩展源码升至 3.7.5：修复 Future Tools newsletter 邮箱误填、余额耗尽报错、无回执迁移种子误跳过，以及成功账本写入失败仍标成功的问题。旧本机 `.env` 中已删除泄露 Key，未将新 Key 写入仓库。47/47 Node 回归与脚本语法检查通过；Ego 尚需重载扩展并完成真实提交、云端回读与三产品各 30 站验收。详见 `docs/外链提交验收与误填保护-2026-09-24.md`。
...
> 最后更新：2026-09-24｜扩展 3.7.5（源码；Ego 待重载验收）｜DeepSeek 新 Key 已接入 Worker｜路线：Neon 执行账本 + R2 证据 + 本地字段优先、AI 补全

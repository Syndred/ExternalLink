...
- **2026-09-21 / 3.7.1 Neon 流量优化**：Neon 控制台确认 ExternalLink 项目当月网络传输已达 5.62 GB 并触发 402。根因是启动、侧边栏动态刷新和 Submify 核验重复读取含大型外链库的完整快照。Worker 已部署轻量修订接口与单文档读取；插件改为首次全量、后续按修订增量回读，侧边栏冷却 5 秒调为 60 秒，配额错误改为 60 分钟退避。异常正文不推进版本、本地缺文档会强制补拉。44/44 回归通过；生产数据库回读须等额度恢复后完成，不能宣称线上闭环已通过。详见 `docs/Neon流量优化-2026-09-21.md`。
...
> 最后更新：2026-09-21｜扩展 3.7.1（源码与 Worker 已部署；Neon 配额恢复后待线上回读）｜路线：Neon 执行账本 + R2 证据 + 本地字段优先、AI 补全

...
> 最后更新：2026-09-09｜扩展 3.2.0｜路线：Neon 执行账本 + R2 证据 + DOM/多模态混合自动化
...
- **DOM + 多模态混合执行**：原生字段、富文本、ARIA/Radix、自定义下拉、文件输入和多步骤表单先走确定性规则/DOM；动作静默无变化或 DOM 无法完成时使用标注截图和 DeepSeek 视觉模型。模型没有 click/submit 权限，最终提交由代码预检执行。

...
- **2026-09-10 / 3.4.3 站点标记多选**：侧栏与外链库支持多选、单独取消、多个标签展示及按任一标记筛选；兼容旧标记，后台串行切换避免双页面覆盖，自动观察不覆盖人工选择。队列与机会评分检查全部标记。源码及 27 组 Node 回归通过；Chrome 仅回读旧版 3.4.2，CUA 后续只返回菜单且无法恢复页面/截图，3.4.3 重载与实际点选待验收。Google 辅助登录可实现，本次仅核查方案。见 `docs/站点多选与谷歌登录-2026-09-10.md`。

> 最后更新：2026-09-10｜扩展 3.4.2｜路线：Neon 执行账本 + R2 证据 + 表单语义理解与确定性执行
...
> **目录站无验证码可代点提交**（设置里默认开，批量页「仅填表」可关）。普通表单先由现有云端 /plan 理解字段和选项，再走确定性填写、上传与提交；只有路由找不到、必填/校验仍失败、多步骤/自定义控件/富文本/上传预览需要观察时，才升级截图智能体逐步接管。看到新回执才写账本。有验证码的页签留下等人。登录 / OTP / 付费仍停。标准 WordPress 评论仍需另开开关才会代点。
...
- **3.4.2 表单语义与跨项目站点经验**：Paid/Free/Freemium 产品定价字段不再直接判成提交收费。普通填表先由模型读取当前表单，按当前 Profile 规划；模型不确定时保留人工复核，不永久写成 paid/broken。C 站手动标记跨 Profile 保留，自动观察单独记录；共享字段关系和最多 12 个阶段的表单结构存于 siteAnnotations，复用时用 B 的资料并核对当前字段。旧 Profile 学习只提取语义，绝不复制 A 的 value。动态回归通过；Chrome 当前加载仍只确认过 3.4.1，新版重载和真实 A/B 同站验收待完成。见 `docs/表单语义与站点经验-2026-09-10.md`。

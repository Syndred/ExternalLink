...
> 最后更新：2026-09-08｜扩展 3.1.4｜路线：云端单一数据源
...
- Chrome 3.1.3 实机冒烟确认批次可启动且不会崩溃，同时抓到并修复三项运行态问题：内容日志重复、停止后日志标题仍写运行中、后台自动开侧栏违反 Chrome 用户手势限制。修复版本为 3.1.4。
...
- **使用前**：重载扩展 **3.1.4**；云端连接、状态首迁移和媒体首迁移已完成。检查队列质量闸门、「目录站无验证码可代点提交」（默认开）和标准评论代点开关（默认关）。切站后看侧栏当前网站名称/URL 是否对得上，再让自动提交继续。
